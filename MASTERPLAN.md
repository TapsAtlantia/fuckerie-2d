...
# BOOK I — WORLD & STRUCTURE GENERATION (the deepening pass) — Phases 1–20

> Goal of the book: make the *world itself* deeper, more varied, and more legible than Terraria's,
> and give it real destinations (dungeon, evil biomes, underworld, sky islands, oceans) full of
> hand-authored structures and loot — all deterministic and co-op-consistent. This is the requested
> starting point and the foundation every later book stands on.

### Phase 1 — Tile-system foundation & 16-bit tiles
...
### Phase 3 — Surface landform realism & deterministic rivers
- **Goal:** A believable planetary surface: plains flats, rolling hills, tall mountains with cliffs,
  plateaus, valleys, beaches, and deterministic rivers/lakes carved into the terrain.
- **Why:** Current surface is gentle rolling + tectonic bumps; the `mountainHeight`/`canyonDepth`
  fields in `Noise.ts` are unused. Terrain variety drives exploration and biome identity.
- **Prereqs:** 1.
- **Touch/Create:** [world/Noise.ts](src/world/Noise.ts) (wire `mountainHeight`/`canyonDepth`, add
  ridged multi-octave landform composition, plateau/cliff shaping), [world/WorldGen.ts](src/world/WorldGen.ts)
  (apply per-biome amplitude from `Biome.surfaceAmplitudeAt`, carve river channels, deposit beaches
  where land meets ocean bands), [world/Liquid.ts](src/world/Liquid.ts) (rivers/lakes fill).
- **Do:**
  1. Compose surface elevation from continent-scale + mountain (ridged) + hill + detail octaves;
     make mountain biomes actually mountainous, valleys low, using elevation coupling that already
     exists in `Biome.selectPrimaryBiome`.
  2. Add cliffs/plateaus via elevation quantization thresholds; beaches (sand) in a band around
     sea level near oceans.
  3. Deterministic rivers: trace low-elevation channels (a river noise field gated by elevation)
     that carve a water channel from highlands toward oceans; fill with static water. Keep it a pure
     function of x.
  4. Keep everything a pure function of world-x (surface) — no per-chunk state.
- **Data:** new noise tunables in `config.ts` (mountain amplitude, ridge sharpness, river density,
  beach width, sea level).
- **MP:** deterministic — no netcode.
- **Done when:** walking the surface shows distinct flats/mountains/valleys/beaches and occasional
  rivers; two peers on the same seed see identical terrain.

### Phase 4 — Ore & vein rework (tiered, depth-gated, hardmode-reserved)
- **Goal:** A real ore progression: tin/copper shallow → iron/lead → silver/tungsten → gold/platinum
  deep, gems in gem pockets, with vein shapes and depth/biome gating, and ids reserved for hardmode
  ores (cobalt/mythril/adamantite/…).
- **Why:** Ore progression gates tool/armor tiers (Book II/III). Current ores are lattice specks
  weighted by biome only.
- **Prereqs:** 1.
- **Touch/Create:** [world/Ores.ts](src/world/Ores.ts), [world/Tile.ts](src/world/Tile.ts) (new ore
  ids + reserve hardmode ore ids), [world/WorldGen.ts](src/world/WorldGen.ts) (call ore pass),
  `config.ts` (`ORE` table per ore: depth band, rarity, vein size, biome bias).
- **Do:**
  1. Define an ore table (id, min/max depth, base rarity, vein size, biome multipliers). Deep ores
     only appear deep; surface ores taper off with depth.
  2. Blob/vein generation (small clustered veins, not single specks), deterministic from hashed vein
     centers; gems in cavern gem pockets.
  3. Reserve hardmode ore tile ids + bar ids now (generated later, in Hardmode phase) so the tile
     space is stable.
- **Data:** ~8–12 pre-hardmode ores + gems; ids reserved for ~6 hardmode ores.
- **MP:** deterministic — no netcode.
- **Done when:** mining down shows a sensible ore curve (shallow copper, deep gold), veins look like
  veins, gems appear in cavern pockets.

### Phase 5 — Cave-system deep rework (spaghetti + cheese + caverns)
- **Goal:** Terraria-grade cave networks: thin winding "spaghetti" tunnels, big "cheese" caverns,
  connected systems with entrances, gated by depth so the surface stays mostly intact.
- **Why:** Caves are where the game is played. Current caves are domain-warped tunnels + rooms; make
  them a legible, connected, explorable network with large open caverns deep down.
- **Prereqs:** 1, 2 (walls behind caves), 3 (surface for cave mouths).
- **Touch/Create:** [world/Caves.ts](src/world/Caves.ts), [world/WorldGen.ts](src/world/WorldGen.ts),
  `config.ts` (`CAVE`).
- **Do:**
  1. Two carve fields: high-frequency worm tunnels (spaghetti) + low-frequency large-threshold
     caverns (cheese), combined so deep areas open into big rooms while shallow areas are tunnels.
  2. Depth-scaled cave density (rare near surface, common in caverns, huge in the deep) so the
     surface reads solid with occasional organic mouths (already partly done — keep the mouth logic).
  3. Ensure connectivity feel (avoid isolated bubbles) by biasing carve fields to be continuous.
  4. Leave background walls intact behind carved caves (Phase 2 output).
- **Data:** cave tunables per depth band.
- **MP:** deterministic.
- **Done when:** descending goes tunnels → rooms → big caverns; caves feel connected and hand-made,
  not speckled; surface has occasional cave mouths.

### Phase 6 — Underground biomes
- **Goal:** Distinct underground regions: Underground Jungle (mud + jungle grass + spore), Ice caves,
  Underground Desert (sandstone caverns), Marble & Granite caves, Glowing Mushroom biome, and the
  deep transition to Caverns.
- **Why:** Terraria's underground is not uniform stone; regional underground biomes drive loot,
  enemies, and mining variety.
- **Prereqs:** 1, 4, 5. Uses existing `UNDERGROUND_BIOMES` in [world/Biome.ts](src/world/Biome.ts).
- **Touch/Create:** [world/Biome.ts](src/world/Biome.ts) (expand `UNDERGROUND_BIOMES`, region
  selection by surface biome above + depth), [world/WorldGen.ts](src/world/WorldGen.ts) (place
  biome stone/grass/props), [world/Tile.ts](src/world/Tile.ts) (marble, granite, mud, spore,
  ice-brick, sandstone-cave tiles + glowing mushroom).
- **Do:**
  1. Extend underground biome selection to inherit from the surface biome column (jungle above →
     underground jungle below) plus depth, with region noise for marble/granite pockets.
  2. Generate biome-specific stone/grass/vegetation; glowing mushroom biome emits light.
  3. Keep transitions readable (biome walls from Phase 2).
- **Data:** ~6 underground biomes with tile sets + light emission for mushroom/crystal.
- **MP:** deterministic.
- **Done when:** digging under a jungle reaches an underground jungle; marble/granite pockets appear;
  glowing mushroom biome glows.

### Phase 7 — Evil biomes: Corruption & Crimson (+ reserve Hallow)
- **Goal:** One evil biome per world (Corruption *or* Crimson, seed-chosen), with their tiles,
  surface chasms, underground variants, demon/crimson altars, and shadow orbs / crimson hearts.
- **Why:** Evil biomes are central to Terraria's identity and progression (altars, boss summons,
  world-evil ore). Reserve the Hallow (post-hardmode) tile ids too.
- **Prereqs:** 1, 2, 3, 6.
- **Touch/Create:** [world/Biome.ts](src/world/Biome.ts) (evil biome as a special surface region),
  [world/WorldGen.ts](src/world/WorldGen.ts) (place evil biome band + chasms + altars),
  [world/Tile.ts](src/world/Tile.ts) (ebonstone/crimstone, corrupt/crimson grass, vile/vicious
  blocks, altars, orbs/hearts + reserve pearl/hallow tiles), structure templates for chasms.
- **Do:**
  1. Seed picks Corruption or Crimson. Place an evil band on the surface (with vertical chasms
     descending into the world) at a deterministic distance from spawn.
  2. Underground evil variant continues the corruption/crimson stone downward.
  3. Place 3 altars in the evil biome (used later to break for hardmode ore + as crafting stations),
     and shadow orbs / crimson hearts as breakable loot nodes.
  4. Reserve the tile-spread machinery (Phase 17 implements spreading).
- **Data:** ~15 evil tiles ×(corruption/crimson) + altars + orbs/hearts; reserved hallow tiles.
- **MP:** deterministic gen; altar/orb breaking is an edit (replicated) that later triggers events.
- **Done when:** each world has exactly one evil biome with chasms + altars + orbs; the other evil's
  ids exist but don't generate.
...
### Phase 9 — The Underworld / Hell
- **Goal:** A deep hell layer: lava seas, ash blocks, hellstone ore, obsidian, ruined houses with
  shadow chests + hellforge, ambient fire glow — the deepest reachable band.
- **Why:** Endgame-of-pre-hardmode destination; Wall of Flesh arena (Phase 52) lives here; hellstone
  → molten tier gear.
- **Prereqs:** 1, 2, 5, 13/14 (structures + loot; stub loot ok).
- **Touch/Create:** [world/WorldGen.ts](src/world/WorldGen.ts) (underworld band gen at
  `BAND.UNDERWORLD`), [world/Tile.ts](src/world/Tile.ts) (ash, hellstone already exists, obsidian,
  hellforge, ruined-house bricks), structure templates for ruined houses,
  [world/Liquid.ts](src/world/Liquid.ts) (lava seas).
- **Do:**
  1. Convert the deep band into an underworld: ash terrain, lava lakes/seas, hellstone veins,
     ambient red light (hellstone already emits light).
  2. Ruined houses (prefab) with shadow chests (hell loot) and a hellforge crafting station tile.
  3. Ensure the band is reachable via caves (connectivity) and is deterministic.
- **Data:** underworld tiles + ruined-house templates + hell loot pool + hellforge station.
- **MP:** deterministic; lava is static (host-authoritative once liquids re-enabled in Phase 64).
- **Done when:** descending far enough reaches hell with lava, hellstone, and ruined houses.
